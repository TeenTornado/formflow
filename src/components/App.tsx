...
function App() {
  ...
  const handleFormAction = (formId: string, action: string) => {
    switch (action) {
      ...
      case 'duplicate':
        ...
        break;
      case 'rename':
        const newName = prompt('Enter new name:', forms.find(f => f.id === formId)?.title);
        if (newName) {
  ...
  const getBreadcrumbItems = () => {
    const items = [{ label: 'Workspaces', onClick: () => setIsCreatingForm(false) }];
    ...
    if (isCreatingForm && currentForm) {
      items.push({ label: currentForm.title });
    }
  ...
  if (isCreatingForm) {
    if (showFormResults) {
      return (
        <FormResults
          form={currentForm!}
          onBack={() => setShowFormResults(false)}
        />
      );
    }
    return (
